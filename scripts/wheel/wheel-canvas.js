...
const PAD=10;
let rowH=layout&&layout.clientHeight>80?layout.clientHeight:areaH;
if(rowH<240)rowH=Math.max(bodyH-headerH-bottomH-modelH,400);
let centerAvail=center.clientHeight>80?center.clientHeight:rowH;
const maxDrawH=Math.max(Math.min(rowH,centerAvail)-legH-PAD,260);
const availW=Math.max(blockW-MIN_SIDE*2,280);
let drawn=Math.floor(Math.min(availW,maxDrawH));
if(maxDrawH<availW)drawn=Math.min(Math.floor(drawn*WHEEL_GROW),maxDrawH,availW);
else drawn=Math.min(drawn,availW,maxDrawH);
drawn=Math.max(drawn,260);
...
ctx.closePath();
const rimG=ctx.createLinearGradient(cx-rimOut,cy-rimOut*1.25,cx+rimOut*0.65,cy+rimOut*0.85);
rimG.addColorStop(0,'#788898');rimG.addColorStop(0.28,'#3a4858');rimG.addColorStop(0.52,'#141c26');rimG.addColorStop(0.72,'#2e3848');rimG.addColorStop(1,'#5a6878');
ctx.fillStyle=rimG;ctx.fill();
ctx.strokeStyle='rgba(0,0,0,0.82)';ctx.lineWidth=2.2;ctx.stroke();
ctx.beginPath();ctx.arc(cx,cy,rimIn,0,Math.PI*2);
ctx.strokeStyle='rgba(255,255,255,0.08)';ctx.lineWidth=1.1;ctx.stroke();
ctx.beginPath();ctx.arc(cx,cy,rimOut,Math.PI*1.02,Math.PI*1.58);
ctx.strokeStyle='rgba(255,255,255,0.2)';ctx.lineWidth=rimOut*0.032;ctx.lineCap='round';ctx.stroke();
for(let i=0;i<wheel.length;i++){
const num=wheel[i];
const start=i*segment-Math.PI/2;
const end=start+segment;
const st=qwEuroPocketStyle(num,lastN);
const midAng=(start+end)*0.5;
const gx=cx+Math.cos(midAng)*((pocketIn+pocketOut)*0.5);
const gy=cy+Math.sin(midAng)*((pocketIn+pocketOut)*0.5);
ctx.beginPath();
ctx.arc(cx,cy,pocketOut,start,end);
ctx.arc(cx,cy,pocketIn,end,start,true);
ctx.closePath();
if(num===lastN)ctx.fillStyle=st.fill;
else{
const pg=ctx.createRadialGradient(gx,gy,pocketIn*0.92,gx,gy,pocketOut*1.02);
if(num===0){pg.addColorStop(0,'rgba(0,180,100,0.58)');pg.addColorStop(1,'rgba(0,90,50,0.36)');}
else if(reds.includes(num)){pg.addColorStop(0,'rgba(210,40,55,0.58)');pg.addColorStop(1,'rgba(110,15,25,0.4)');}
else{pg.addColorStop(0,'rgba(42,42,54,0.58)');pg.addColorStop(1,'rgba(10,10,16,0.42)');}
ctx.fillStyle=pg;
}
ctx.fill();
...
ctx.save();
ctx.shadowColor='rgba(0,0,0,0.88)';ctx.shadowBlur=40;ctx.shadowOffsetY=10;
ctx.beginPath();ctx.arc(cx,cy+8,outerR*0.98,0,Math.PI*2);
ctx.fillStyle='rgba(0,0,0,0.52)';ctx.fill();
ctx.shadowBlur=28;ctx.shadowOffsetY=9;
ctx.beginPath();ctx.arc(cx,cy+5,outerR*0.98,0,Math.PI*2);
ctx.fillStyle='rgba(0,0,0,0.22)';ctx.fill();
ctx.shadowBlur=0;ctx.shadowOffsetY=0;
const baseR=outerR*1.008;
ctx.beginPath();ctx.arc(cx,cy,baseR,0,Math.PI*2);
const baseG=ctx.createRadialGradient(cx,cy-baseR*0.38,baseR*0.08,cx,cy+baseR*0.22,baseR*1.08);
baseG.addColorStop(0,'#324458');baseG.addColorStop(0.42,'#182028');baseG.addColorStop(0.76,'#0a1018');baseG.addColorStop(1,'#243040');
ctx.fillStyle=baseG;ctx.fill();
ctx.strokeStyle='rgba(70,140,190,0.28)';ctx.lineWidth=1.6;ctx.stroke();
const glow=ctx.createRadialGradient(cx,cy,outerR*0.72,cx,cy,outerR*1.06);
glow.addColorStop(0,'rgba(40,160,255,0)');glow.addColorStop(0.62,'rgba(40,160,255,'+(0.06+0.04*pulse)+')');
glow.addColorStop(0.88,'rgba(66,255,116,'+(0.14+0.06*pulse)+')');glow.addColorStop(1,'rgba(0,0,0,0)');
ctx.fillStyle=glow;ctx.beginPath();ctx.arc(cx,cy,outerR*1.06,0,Math.PI*2);ctx.fill();
ctx.beginPath();ctx.arc(cx,cy,outerR*1.018,0,Math.PI*2);
ctx.strokeStyle='rgba(80,210,255,'+(0.22+0.12*pulse)+')';ctx.lineWidth=2.6;ctx.stroke();
ctx.beginPath();ctx.arc(cx,cy,outerR*1.012,0,Math.PI*2);
ctx.strokeStyle='rgba(66,255,116,'+(0.1+0.06*pulse)+')';ctx.lineWidth=1.3;ctx.stroke();
ctx.beginPath();ctx.arc(cx,cy,outerR*1.03,Math.PI*1.04,Math.PI*1.64);
ctx.strokeStyle='rgba(255,255,255,'+(0.32+0.1*pulse)+')';ctx.lineWidth=outerR*0.042;ctx.lineCap='round';ctx.stroke();
ctx.beginPath();ctx.arc(cx,cy,outerR*0.985,Math.PI*0.12,Math.PI*0.88);
ctx.strokeStyle='rgba(0,0,0,0.42)';ctx.lineWidth=outerR*0.028;ctx.lineCap='round';ctx.stroke();
ctx.save();
ctx.globalCompositeOperation='lighter';
ctx.globalAlpha=0.34+0.1*pulse;
const dome=ctx.createRadialGradient(cx-outerR*0.24,cy-outerR*0.34,0,cx,cy,outerR*0.96);
dome.addColorStop(0,'rgba(200,240,255,0.62)');dome.addColorStop(0.32,'rgba(90,190,255,0.14)');dome.addColorStop(1,'rgba(0,0,0,0)');
ctx.fillStyle=dome;ctx.beginPath();ctx.arc(cx,cy,outerR*0.94,0,Math.PI*2);ctx.fill();
ctx.restore();
ctx.restore();
}
...
ctx.save();
ctx.globalCompositeOperation='lighter';
const spec=ctx.createRadialGradient(cx-hubR*0.14,cy-hubR*0.22,0,cx-hubR*0.14,cy-hubR*0.22,hubR*0.38);
spec.addColorStop(0,'rgba(255,255,255,0.92)');spec.addColorStop(0.35,'rgba(170,230,255,0.28)');spec.addColorStop(1,'rgba(0,0,0,0)');
ctx.fillStyle=spec;ctx.beginPath();ctx.arc(cx-hubR*0.1,cy-hubR*0.18,hubR*0.3,0,Math.PI*2);ctx.fill();
ctx.restore();
const hubShell=ctx.createRadialGradient(cx,cy-hubR*0.12,hubR*0.05,cx,cy,hubR*0.2);
hubShell.addColorStop(0,'#1a3048');hubShell.addColorStop(0.55,'#0a1420');hubShell.addColorStop(1,'#02060c');
ctx.fillStyle=hubShell;ctx.beginPath();ctx.arc(cx,cy,hubR*0.18,0,Math.PI*2);ctx.fill();
ctx.strokeStyle='rgba(130,230,255,0.62)';ctx.lineWidth=1.2;ctx.stroke();
ctx.strokeStyle='rgba(255,255,255,0.12)';ctx.lineWidth=0.6;ctx.beginPath();ctx.arc(cx,cy,hubR*0.16,Math.PI*1.1,Math.PI*1.55);ctx.stroke();
ctx.restore();
...
const outerR=radarMinimal?Math.min(half-16,minDim*0.496):minDim*0.47;
