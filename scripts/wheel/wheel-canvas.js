...
ctx.save();
ctx.shadowColor='rgba(0,0,0,0.88)';ctx.shadowBlur=52;ctx.shadowOffsetY=18;
ctx.beginPath();ctx.arc(cx,cy+12,outerR*0.96,0,Math.PI*2);
...
const glow=ctx.createRadialGradient(cx,cy,outerR*0.68,cx,cy,outerR*1.18);
...
ctx.fillStyle=glow;ctx.beginPath();ctx.arc(cx,cy,outerR*1.18,0,Math.PI*2);ctx.fill();
ctx.beginPath();ctx.arc(cx,cy,outerR*1.035,0,Math.PI*2);
ctx.strokeStyle='rgba(80,210,255,'+(0.22+0.12*pulse)+')';ctx.lineWidth=3.2;ctx.stroke();
