/**
 * Komplexná kontrola slovenčiny + všetky V2 testy (súhrn)
 * Spustenie: npm run test:v2:all  (volá test:v4, test:v2:audit, test:v2:wheel)
 */
const fs = require('fs');
...
if (fs.readFileSync(indexPath, 'utf8').length !== html.length) {
  fail('index.html nie je sync — spusti: npm run sync-v2');
} else ok('sync index.html ↔ V2');
...
app.whenReady().then(async () => {
  ...
  if (failed === 0) {
    ...
    runNpmScript('test:v2:audit');
    runNpmScript('test:v2:wheel');
  }
