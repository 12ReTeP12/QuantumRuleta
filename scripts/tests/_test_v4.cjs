/**
 * Layout V6 + koleso UI (V2) (základný)
 * Spustenie: npm run test:v2:layout
 */
const fs = require('fs');
...
if (fs.existsSync(path.join(root, 'index.html'))) {
  ...
  if (idx.length !== html.length) fail('index.html != V2 (spusti: npm run sync-v2)');
  else ok('index.html sync s V2');
} else ok('index.html sync s V2 (iba V4)');
...
app.whenReady().then(async () => {
  ...
  const report = await win.webContents.executeJavaScript(`(function(){
    ...
    check('koleso radar V1 + canvas', () => {
      ...
      const okUi = $('wheelCanvas') && $('qwStatusBanner') && left.includes('FLOW STAV') && n >= 5;
