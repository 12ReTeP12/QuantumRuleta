/**
 * Komplexný runtime test V2 — observer, upozornenia, reset, enginy
 * Spustenie: npm run test:v2
 */
const fs = require('fs');
const path = require('path');
const { app, BrowserWindow } = require('electron');

const htmlPath = path.join(__dirname, '..', '..', 'index-NOVY-V2.html');
const v4Html = fs.readFileSync(htmlPath, 'utf8');
let failed = 0;
const ok = (m) => console.log('OK:', m);
const fail = (m) => { console.error('FAIL:', m); failed++; };
ok('V4 autoritatívny — index-NOVY-V2.html');
